...
function emitOperation(op: Operation): string {
  ...
  switch (op.type) {
    ...
    case 'ID':
      return `# ID gate skipped (or use circ.noop(${q[0]}))`;

    // Parameterized single-qubit
    case 'RX':
      return `circ.Rx(${f(getParam(op, 'theta', Math.PI / 2))}, ${q[0]})`;
    case 'RY':
      return `circ.Ry(${f(getParam(op, 'theta', Math.PI / 2))}, ${q[0]})`;
    case 'RZ':
      return `circ.Rz(${f(getParam(op, 'theta', Math.PI / 4))}, ${q[0]})`;
    case 'P':
      return `circ.U1(${f(getParam(op, 'lambda', Math.PI / 4))}, ${q[0]})`;
    case 'U':
      return `circ.U3(${f(getParam(op, 'theta', Math.PI / 2))}, ${f(getParam(op, 'phi', 0))}, ${f(getParam(op, 'lambda', Math.PI))}, ${q[0]})`;
    ...
    // Measurement
    case 'MEASURE':
      return `circ.Measure(${q[0]}, ${op.targets.clbits![0]})`;

    default:
      return `# Unsupported gate ${op.type} in TKET`;
  }
}
