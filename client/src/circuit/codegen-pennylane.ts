...
export function generatePennyLaneCode(circuit: CircuitModel): string {
  ...
  if (hasMeasure) {
    const measStr = Array.from(measuredQubits)
      .map((q) => `qml.probs(wires=[${q}])`)
      .join(', ');
...
function emitOperation(op: Operation): string {
  ...
  switch (op.type) {
    ...
    case 'SDG':
      return `qml.adjoint(qml.S)(wires=[${q[0]}])`;
    case 'TDG':
      return `qml.adjoint(qml.T)(wires=[${q[0]}])`;
    case 'SXDG':
      return `qml.adjoint(qml.SX)(wires=[${q[0]}])`;
    case 'ID':
      return `qml.Identity(wires=[${q[0]}])`;

    // Parameterized single-qubit
    case 'RX':
      return `qml.RX(${f(getParam(op, 'theta', Math.PI / 2))}, wires=[${q[0]}])`;
    case 'RY':
      return `qml.RY(${f(getParam(op, 'theta', Math.PI / 2))}, wires=[${q[0]}])`;
    case 'RZ':
      return `qml.RZ(${f(getParam(op, 'theta', Math.PI / 4))}, wires=[${q[0]}])`;
    case 'P':
      return `qml.PhaseShift(${f(getParam(op, 'lambda', Math.PI / 4))}, wires=[${q[0]}])`;
    case 'U':
      return `qml.U3(${f(getParam(op, 'theta', Math.PI / 2))}, ${f(getParam(op, 'phi', 0))}, ${f(getParam(op, 'lambda', Math.PI))}, wires=[${q[0]}])`;
    ...
    case 'CRX':
      return `qml.CRX(${f(getParam(op, 'theta', Math.PI / 2))}, wires=[${q[0]}, ${q[1]}])`;
    case 'CRY':
      return `qml.CRY(${f(getParam(op, 'theta', Math.PI / 2))}, wires=[${q[0]}, ${q[1]}])`;
    case 'CRZ':
      return `qml.CRZ(${f(getParam(op, 'theta', Math.PI / 4))}, wires=[${q[0]}, ${q[1]}])`;
    case 'CP':
      return `qml.ControlledPhaseShift(${f(getParam(op, 'lambda', Math.PI / 4))}, wires=[${q[0]}, ${q[1]}])`;
    ...
    default:
      return `# Unsupported gate ${op.type} in PennyLane`;
  }
}
