...
function emitOperation(op: Operation): string {
  ...
  switch (op.type) {
    ...
    case 'SDG':
      return `circuit.si(${q[0]})`;
    case 'TDG':
      return `circuit.ti(${q[0]})`;
    case 'SX':
      return `circuit.v(${q[0]})`; // V gate is sqrt(X)
    case 'SXDG':
      return `circuit.vi(${q[0]})`;
    case 'ID':
      return `circuit.i(${q[0]})`;

    // Parameterized single-qubit
    case 'RX':
      return `circuit.rx(${q[0]}, ${f(getParam(op, 'theta', Math.PI / 2))})`;
    case 'RY':
      return `circuit.ry(${q[0]}, ${f(getParam(op, 'theta', Math.PI / 2))})`;
    case 'RZ':
      return `circuit.rz(${q[0]}, ${f(getParam(op, 'theta', Math.PI / 4))})`;
    case 'P':
      return `circuit.phaseshift(${q[0]}, ${f(getParam(op, 'lambda', Math.PI / 4))})`;
    case 'U':
    ...
    default:
      return `# Unsupported gate ${op.type} in Braket`;
  }
}
