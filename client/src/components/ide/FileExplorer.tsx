...
const QasmIcon = () => (
  <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#10b981" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
...
const ChevronIcon = ({ open }: { open?: boolean }) => (
  <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ transform: open ? 'rotate(90deg)' : 'none', transition: 'transform 0.1s', color: 'var(--color-text-subtle)', flexShrink: 0 }}>
...
export default function FileExplorer({
  ...
  const handleStartCreate = (type: 'file' | 'folder', parentId: string | null, e: React.MouseEvent) => {
    e.stopPropagation();
      ...
      <div key={node.id}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flex: 1, minWidth: 0 }}>
            {isFolder && <div style={{ display: 'flex', alignItems: 'center', width: '12px' }}><ChevronIcon open={isExpanded} /></div>}
        ...
        {isFolder && isExpanded && creatingParentId === node.id && (
          <div style={{ display: 'flex', alignItems: 'center', padding: `4px 8px 4px ${paddingLeft + 12}px`, height: '28px' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', paddingBottom: '12px', flex: '1 1 auto', overflowY: 'auto' }}>
      ...
      {creatingParentId === null && creatingType && (
        <div style={{ display: 'flex', alignItems: 'center', padding: '4px 8px 4px 12px', height: '28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flex: 1 }}>
            <div style={{ width: '12px' }} />
            {creatingType === 'folder' ? <FolderIcon /> : <FileIcon />}
            <input
