...
export default function CircuitProfilerPanel({ circuit }: CircuitProfilerPanelProps) {
  ...
  const depth = gateCount > 0 ? Math.max(...circuit.operations.map((o) => o.time)) + 1 : 0;

  // Calculate how many multi-qubit gates exist (costly on NISQ hardware)
  const multiQubitGates = circuit.operations.filter((o) => GATE_QUBIT_COUNT[o.type] > 1).length;
  ...
  const estimatedFidelity =
    gateCount === 0
      ? 100
      : Math.max(
          0,
          Math.round(Math.pow(0.99, multiQubitGates) * Math.pow(0.999, singleQubitGates) * 100),
        );
    ...
    <div className="profiler-panel">
      ...
      <div className="profiler-panel__metrics">
        <div className="profiler-panel__metric">
          ...
          <span
            className={`profiler-panel__metric-value ${isDeep ? 'profiler-panel__metric-value--warn' : ''}`}
          >
            {depth}
          </span>
        </div>
        ...
        <div className="profiler-panel__metric">
          ...
          <span
            className={`profiler-panel__metric-value ${isNoisy ? 'profiler-panel__metric-value--error' : 'profiler-panel__metric-value--success'}`}
          >
            {estimatedFidelity}%
          </span>
        </div>
      ...
      {(isDeep || isNoisy) && (
        <div className="profiler-panel__warning">
          <strong>Hardware Warning:</strong> This circuit is deep and contains many multi-qubit
          gates. It may suffer from significant noise and decoherence on actual NISQ hardware.
          Consider optimizing your circuit.
        </div>
      )}
    </div>
  );
