...
export default function TranspilationEngine({ isOpen, onClose, circuit }: Props) {
  ...
  const isOnFinalPass = engine.selectedGlobalIndex === engine.flatPasses.length - 1 && engine.flatPasses.length > 0;
      ...
      <div className="te-window">
        ...
        <header className="te-header">
          <div className="te-header-left">
            <span className="te-header-icon" aria-hidden="true">⚛</span>
            <h2 id="te-title" className="te-header-title">Transparent Transpilation Engine</h2>
        ...
        {engine.status === 'ready' && engine.trace && (
          <div className="te-stage-bar" role="navigation" aria-label="Transpilation stages">
            ...
            {engine.trace.stages.filter(s => s.passes.length > 0).map((stage, si) => {
        ...
        <div className="te-body">
          ...
          {engine.status === 'error' && (
            <div className="te-error">
              ...
              <p>{engine.error}</p>
              <button className="btn btn--primary" onClick={engine.run}>Retry</button>
            </div>
          )}
            ...
            <div className="te-main-layout">
              ...
              {['Mapping', 'Routing', 'Scheduling'].includes(engine.selectedPass.stageName) && (
                <aside className="te-hardware-panel">
                  <TranspileHardwareMap
                    trace={engine.trace}
                    selectedPass={engine.selectedPass}
                  />
                </aside>
              )}
            </div>
        ...
        {engine.status === 'ready' && engine.flatPasses.length > 0 && (
          <footer className="te-footer">
            <div className="te-playback">
              ...
              {engine.isPlaying ? (
                <button className="btn btn--primary btn--sm te-pb-play" onClick={engine.pause} aria-label="Pause">
                  ⏸ Pause
                </button>
              ) : (
                <button className="btn btn--primary btn--sm te-pb-play" onClick={engine.play} aria-label="Play">
                  ▶ Play
                </button>
              )}
              ...
              <div className="te-pb-progress">
                <div
                  ...
                  style={{ width: `${((engine.selectedGlobalIndex + 1) / engine.flatPasses.length) * 100}%` }}
                />
              </div>

              <span className="te-pb-label">
                {engine.selectedPass?.stageName ?? ''} · Pass {engine.selectedGlobalIndex + 1}/{engine.flatPasses.length}
              </span>
            </div>
          </footer>
        )}
