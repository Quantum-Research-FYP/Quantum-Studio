...
const PHASES = [
  ...
  { id: 'analysis', name: 'Circuit Analysis', desc: 'Analyzing depth, gates, and qubit dependencies.' },
  { id: 'optimization', name: 'Optimization', desc: 'Canceling adjacent inverse gates and consolidating 1Q operations.' },
  { id: 'basis', name: 'Basis Conversion', desc: 'Unrolling into the native gate set (e.g., CX, ID, RZ, SX, X).' },
  { id: 'mapping', name: 'Qubit Mapping', desc: 'Assigning logical qubits to physical hardware qubits.' },
  { id: 'routing', name: 'Routing', desc: 'Inserting SWAP gates to satisfy hardware coupling map constraints.' },
  { id: 'scheduling', name: 'Scheduling', desc: 'Aligning gates with hardware instruction timing.' },
  { id: 'final', name: 'Final Hardware Circuit', desc: 'Transpilation complete. Circuit is ready for execution.' }
];
...
export default function TranspilationSimulationModal({
  ...
  const displayCircuit: CircuitModel = useMemo(() => {
    ...
    if (currentPhaseIndex >= 3) {
      newCircuit.operations = newCircuit.operations.map((op) => {
        if (op.targets.qubits.length === 1 && !['RZ', 'SX', 'X', 'ID', 'MEASURE'].includes(op.type)) {
          return { ...op, type: 'SX' };
        }
    ...
    if (currentPhaseIndex >= 5 && newCircuit.qubits > 1) {
      ...
      const maxTime = newCircuit.operations.length > 0 ? Math.max(...newCircuit.operations.map(o => o.time)) : 0;
  ...
  return (
    <div className="transpilation-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="transpilation-title">
      <div className="transpilation-modal-content">
        <header className="transpilation-modal-header">
          ...
          <button className="btn btn--ghost btn--sm transpilation-close" onClick={onClose} aria-label="Close modal">✕</button>
        </header>
        ...
        <div className="transpilation-modal-body">
            ...
            {PHASES.map((phase, index) => {
              ...
              return (
                <div key={phase.id} className={`transpilation-step ${statusClass}`}>
                  <div className="transpilation-step-indicator">
                    {isPast ? '✓' : index + 1}
                  </div>
          ...
          <div className="transpilation-visualizer">
             <div className="transpilation-visualizer-circuit phase-transition" style={{ pointerEvents: 'none' }}>
               <CircuitCanvas 
