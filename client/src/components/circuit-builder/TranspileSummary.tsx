...
export default function TranspileSummary({ trace, originalCircuit, onClose }: Props) {
  ...
  return (
    <div className="te-summary">
      <div className="te-summary-icon" aria-hidden="true">✓</div>
      ...
      <div className="te-summary-grid">
        ...
        <div className="te-summary-card te-summary-card--original">
          ...
          <div className="te-summary-stat">
            <span>Qubits</span><strong>{trace.originalGateCount > 0 ? originalCircuit.qubits : '—'}</strong>
          </div>
          <div className="te-summary-stat">
            <span>Gates</span><strong>{trace.originalGateCount}</strong>
          </div>
          <div className="te-summary-stat">
            <span>Depth</span><strong>{trace.originalDepth}</strong>
          </div>
          <div className="te-summary-stat">
            <span>2Q Gates</span><strong>{orig2Q}</strong>
          </div>
        </div>

        {/* Arrow */}
        <div className="te-summary-arrow" aria-hidden="true">→</div>

        {/* Final */}
        <div className="te-summary-card te-summary-card--final">
          <h4>Final Hardware Circuit</h4>
          <div className="te-summary-stat">
            <span>Qubits</span><strong>{originalCircuit.qubits}</strong>
          </div>
          <div className="te-summary-stat">
            <span>Gates</span><strong>{trace.finalGateCount}</strong>
          </div>
          <div className="te-summary-stat">
            <span>Depth</span><strong>{trace.finalDepth}</strong>
          </div>
          <div className="te-summary-stat">
            <span>2Q Gates</span><strong>—</strong>
          </div>
        </div>
      </div>
      ...
      <div className="te-summary-changes">
        ...
        <div className="te-summary-change-row">
          ...
          <span className={`te-summary-change-val ${gateChange.direction === 'decrease' ? 'good' : 'warn'}`}>
            {gateChange.label} {gateChange.pct}
          </span>
        </div>
        <div className="te-summary-change-row">
          <span>Circuit depth</span>
          <span className={`te-summary-change-val ${depthChange.direction === 'decrease' ? 'good' : 'warn'}`}>
